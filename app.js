...
authDomain: “DEIN_PROJECT_ID.firebaseapp.com”,  // ← Ersetze DEIN_PROJECT_ID
projectId: “DEIN_PROJECT_ID”,                   // ← Ersetze DEIN_PROJECT_ID
storageBucket: “DEIN_PROJECT_ID.appspot.com”,   // ← Ersetze DEIN_PROJECT_ID
messagingSenderId: “DEINE_SENDER_ID”,           // ← Ersetze mit deiner Sender ID
appId: “DEINE_APP_ID”                           // ← Ersetze mit deiner App ID
};
...
db.version(1).stores({
cars: ‘id, userId, name, createdAt’,
fillups: ‘id, userId, carId, datetime’,
settings: ‘userId’
});
...
toast.innerHTML = `<span style="font-size: 20px;"> ${type === 'success' ? '✅' : type === 'error' ? '❌' : type === 'warning' ? '⚠️' : 'ℹ️'} </span> <span>${message}</span>`;
...
const fillups = await db.fillups
.where(‘carId’).equals(carId)
.toArray();
...
// Sync fillups
const fillupsSnap = await firestore
  .collection('users')
  .doc(uid)
  .collection('fillups')
  .get();
...
if (password.length < 6) {
  authError.textContent = 'Passwort muss mindestens 6 Zeichen lang sein';
  authError.classList.remove('hidden');
  return;
}
...
try {
  if (useCloud && app.firebase) {
    ...
    app.user = { uid: userCredential.user.uid, email: email };
    app.useCloud = true;
    await syncFromCloud();
  } else {
    // Local only authentication
    const userId = 'local_' + btoa(email).replace(/[^a-zA-Z0-9]/g, '');
    
    if (authMode === 'register') {
      // Check if user already exists locally
      const existingUser = localStorage.getItem('user_' + userId);
      if (existingUser) {
        authError.textContent = 'Benutzer existiert bereits';
        authError.classList.remove('hidden');
        return;
      }
      ...
      const localUser = {
        uid: userId,
        email: email,
        password: btoa(password), // Simple encoding (not secure, but OK for local)
        createdAt: new Date().toISOString()
      };
      
      localStorage.setItem('user_' + userId, JSON.stringify(localUser));
      app.user = { uid: userId, email: email };
      app.useCloud = false;
      ...
      if (!storedUser) {
        authError.textContent = 'Benutzer nicht gefunden';
        authError.classList.remove('hidden');
        return;
      }
      ...
      // Check password
      if (localUser.password !== btoa(password)) {
  ...
  localStorage.setItem('useCloud', app.useCloud ? 'true' : 'false');
  startApp();
} catch (error) {
...
if (!app.useCloud) {
showToast(‘Cloud-Sync ist deaktiviert’, ‘warning’);
return;
}
...
container.innerHTML = ‘<div class="fade-in">’ + await getViewContent(view) + ‘</div>’;
...
carThumb.innerHTML = `<img src="${app.currentCar.imageData}" alt="${app.currentCar.name}">`;
} else {
...
carMeta.textContent = `${app.currentCar.fuelType || 'Kraftstoff'} · ${app.currentCar.tankCapacity || '--'}L Tank · ${formatNumber(app.currentCar.avgConsumption || 0, 1)} L/100km`;

const fillups = await getFillups(app.currentCar.id);
if (fillups.length > 0) {
  const avgPrice = fillups.reduce((sum, f) => sum + f.pricePerLiter, 0) / fillups.length;
  carAvgPrice.textContent = formatNumber(avgPrice, 3);
} else {
...
carMeta.textContent = ‘Erstelle dein erstes Auto’;
carAvgPrice.textContent = ‘–’;
}
}
...
async function getViewContent(view) {
switch (view) {
...
if (!app.currentCar) {
return `<div class="card text-center"> <h2>👋 Willkommen bei FuelBook!</h2> <p class="text-muted mb-3">Erstelle dein erstes Auto, um zu starten.</p> <button class="btn btn-primary" onclick="navigateTo('cars')"> 🚗 Auto erstellen </button> </div>`;
}
...
// Calculate stats
const totalSpent = fillups.reduce((sum, f) => sum + f.totalPrice, 0);
const totalLiters = fillups.reduce((sum, f) => sum + f.liters, 0);
const avgPrice = fillups.length > 0 ? totalSpent / totalLiters : 0;
...
<div class="stat-card">
<div class="stat-card-value">${formatNumber(avgPrice, 2)}€</div>
    ...
    <div style="display: flex; flex-direction: column; gap: 12px;">
      ${recentFillups.map(f => `
        <div style="display: flex; align-items: center; gap: 16px; padding: 12px; background: var(--bg-secondary); border-radius: var(--radius-sm); border: 1px solid var(--border);">
          ...
          <div style="flex: 1;">
            <div style="font-weight: 600;">${formatCurrency(f.totalPrice)} · ${formatNumber(f.liters, 1)}L</div>
            <div style="font-size: 14px; color: var(--text-muted);">${formatDate(f.datetime)} · ${formatNumber(f.pricePerLiter, 3)}€/L</div>
          </div>
          <div style="text-align: right;">
            <div style="font-weight: 700; color: var(--accent);">${formatNumber(f.odometer, 0)} km</div>
          </div>
        </div>
      `).join('')}
    </div>
...
if (!app.currentCar) {
return `<div class="card text-center"> <p class="text-muted">Wähle zuerst ein Auto aus</p> <button class="btn btn-primary mt-2" onclick="navigateTo('cars')">Zu Autos</button> </div>`;
}
...
```
  <div class="stats-grid">
    <div class="stat-card">
      <div class="stat-card-value">${formatNumber(avgConsumption, 2)}</div>
    ...
    <div class="stat-card">
      <div class="stat-card-value">${formatNumber(totalLiters, 0)}</div>
      <div class="stat-card-label">Gesamt Liter</div>
    </div>
    <div class="stat-card">
      <div class="stat-card-value">${fillups.length}</div>
  ...
  <div id="carsList" style="display: flex; flex-direction: column; gap: 12px;">
    ${cars.length > 0 ? cars.map(car => `
      <div style="display: flex; align-items: center; gap: 16px; padding: 16px; background: var(--bg-secondary); border-radius: var(--radius-sm); border: 1px solid var(--border);">
        ...
        <div style="flex: 1;">
          <div style="font-weight: 700; font-size: 18px;">${car.name}</div>
          <div style="font-size: 14px; color: var(--text-muted); margin-top: 4px;">
            ${car.fuelType || 'Kraftstoff'} · ${car.tankCapacity || '--'}L Tank · ${formatNumber(car.avgConsumption || 0, 1)} L/100km
          </div>
        </div>
    ...
    `).join('') : `
      <p class="text-muted text-center">Noch keine Autos vorhanden</p>
    `}
  </div>
</div>
...
```
  ...
  <div class="form-group">
    ...
    <div style="padding: 16px; background: var(--bg-secondary); border-radius: var(--radius-sm); border: 1px solid var(--border);">
      <div style="font-weight: 600; margin-bottom: 4px;">${app.user.email}</div>
      <div style="font-size: 14px; color: var(--text-muted);">User ID: ${app.user.uid}</div>
    </div>
  </div>
  ...
  <div class="form-group">
    <button class="btn btn-secondary" style="width: 100%;" onclick="$('#importFile').click()">
  ...
  <div class="form-group">
    <button class="btn" style="width: 100%; background: var(--danger); color: white;" onclick="clearAllData()">
...
```
    const liters = parseFloat($('#fillupLiters').value);
    const price = parseFloat($('#fillupPrice').value);
    ...
    const fillup = {
      ...
      odometer: parseInt($('#fillupOdometer').value),
      station: $('#fillupStation').value || null,
      note: $('#fillupNote').value || null
    };
...
```
    ...
    // Check if we're editing and preserve existing image
    if ($('#carId').value) {
      const existingCar = await db.cars.get($('#carId').value);
      imageData = existingCar?.imageData || null;
    }
    ...
    const car = {
      ...
      tankCapacity: parseFloat($('#carTank').value) || null,
      avgConsumption: parseFloat($('#carConsumption').value) || null,
      imageData: imageData,
      createdAt: new Date().toISOString()
    };
...
const carsRef = firebase.firestore().collection(‘users’).doc(app.user.uid).collection(‘cars’);
const fillupsRef = firebase.firestore().collection(‘users’).doc(app.user.uid).collection(‘fillups’);
...
const labels = sorted.map(f => new Date(f.datetime).toLocaleDateString(‘de-DE’, { month: ‘short’, day: ‘numeric’ }));
const prices = sorted.map(f => f.pricePerLiter);
...
app.useCloud = savedUseCloud;

```
  // Try to restore Firebase session if cloud is enabled
  if (app.useCloud && app.firebase) {
    firebase.auth().onAuthStateChanged(async (user) => {
      if (user) {
        app.user = { uid: user.uid, email: user.email };
        await syncFromCloud();
        startApp();
      } else {
        // Firebase session expired, fallback to local
        app.useCloud = false;
        startApp();
      }
    });
